...
**What it does:** see stages 1–8 below. The script is idempotent.
...
- Runner state (`/var/lib/abhaile/runner/`) is preserved across re-enrollment.
- Rendered output at `/var/lib/abhaile/rendered/` is wiped and regenerated by render.
- A fresh bootstrap token is required (the previous one-time token was consumed).
